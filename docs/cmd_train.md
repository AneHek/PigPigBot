...
handler.py :: handle_message()
    ...
    │  get_handler("训练") → TrainingMixin.start_training  (装饰器注册)
    │
    ▼
game/training.py :: TrainingMixin.start_training(user_id, user_name, arg, group_id)
    │
    ...
    │     └─ data/pet_store.py :: PetStoreMixin.start_training()
    │        ├─ get_pet() → 获取 Pet
    │        ├─ pet.training = True
    ...
    │     └─ pet/stats.py :: calc_training_exp()
    │        └─ 公式: 50 * level * minutes
    │        └─ 计算 10 分钟预计获得经验
    │
...
| `start_training()` | data/pet_store.py | 设置训练状态、记录开始时间 |
| `calc_training_exp()` | pet/stats.py | 计算训练经验公式 |
