...
随机领养一只猪猪宠物（P001~P026），生成 IV、品质、初始属性，返回截图+按钮消息。首次领养自动分配游戏用户ID（从1开始递增）。
...
handler.py :: handle_message()
    ...
    │  handlers["领养"] → game.adopt(user_id, user_name, arg)
    │
    ▼
pet_game.py :: PetGame.adopt(user_id, user_name, _arg)
    │
    ...
    │     │   └─ Redis GET qqbot:user_game_uid:{user_id}
    ...
    │     └─ 若已有 → 复用（遗弃后重新领养保留原ID）
    │
    ├─ 3. random.randint(1, 26) → species_id = f"P{num:03d}"
    │
...
| 函数 | 文件 | 作用 |
|------|------|------|
| `has_pet()` | data_manager.py | 检查用户是否已有宠物 |
| `get_user_game_uid()` | data_manager.py | 获取用户游戏用户ID |
| `assign_game_uid()` | data_manager.py | 原子分配游戏用户ID + 建立双向映射 |
| `generate_quality()` | pet_stats.py | 二项分布生成品质档位 |
| `generate_ivs()` | pet_stats.py | 生成 6 维 IV 个体值 |
| `calc_stats()` | pet_stats.py | 计算战斗属性 |
| `create_pet()` | data_manager.py | 创建宠物并写入 Redis |
| `update_pet()` | data_manager.py | 更新宠物 game_uid 字段 |
| `update_leaderboard()` | data_manager.py | 更新排行榜 |
| `_generate_screenshot()` | pet_game.py | 截图核心 |
